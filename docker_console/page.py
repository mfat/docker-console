...
# The full-colour Docker mark, shipped as package data.
_MARK_FILE = os.path.join(os.path.dirname(__file__), "resources", "icons",
                          "docker-mark-ocean-blue.svg")
...
class DockerConsolePage(
    ...
    def _make_docker_mark(self, size: int = 56) -> Gtk.Image:
        ...
        Loaded as a texture straight from the packaged SVG rather than via the
    ...
    def _build_host_bar(self) -> Gtk.Widget:
        ...
        # Caption clarifying where this docker lives (local now; "via SSH" once
        # a remote host is selected).
        self._via_label = Gtk.Label()
        self._via_label.add_css_class("dim-label")
        self._via_label.add_css_class("caption")
        bar.append(self._via_label)
        self._update_via_label()
    ...
    def _selected_connection(self) -> Optional[Any]:
        nick = self._selected_nick
        for c in (self._connections or []):
            if getattr(c, "nickname", None) == nick:
                return c
        return None
    ...
    def _update_host_button_label(self) -> None:
        ...
        if self._selected_nick:
            self._host_label.set_label(
                getattr(conn, "label", None) or self._selected_nick)
    ...
    def _update_via_label(self) -> None:
        if getattr(self, "_via_label", None) is None:
            return
        conn = self._selected_connection()
        kind = getattr(conn, "kind", "local")
        self._via_label.set_label("" if kind == "local" else "via SSH")
    ...
    def _list_ssh_connections(self) -> List[Any]:
        # The app's registry only holds valid docker targets (local now, remote
        # SSH later), so no protocol filtering — return them all.
        try:
            return list(self.ctx.list_connections())
        except Exception:
            return []
